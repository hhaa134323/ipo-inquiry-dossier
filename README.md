...
> 我的可比公司问询回复 PDF 都放在 `<我放PDF的目录>`（换成你自己的真实路径，比如 Windows 写 `D:\可比公司问询回复`，Mac/Linux 写 `~/可比公司问询回复`），底稿想输出到 `<输出目录>`。请用 ipo-inquiry-dossier 帮我做一份可比先例底稿，要回答的问询是：「结合功率器件代工业务开展情况等分析报告期内功率器件代工产品毛利率未见明显改善的原因及合理性，未来拟采取的改善功率器件代工产品毛利率水平的措施及可实现性。」
