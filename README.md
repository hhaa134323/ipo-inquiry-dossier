...
没有完整标准答案，也能验证这套流程值不值得信。用 7 道真实的第一轮审核问询逐题跑了一遍，都是半导体 IPO 里偏难的题，涉及盈利预测、收入与客户、毛利率等多个口径。每题单独用 DeepSeek V4-Flash 跑完，成本按官方定价折算成人民币。

| 指标 | 结果 | 说明 |
|---|---|---|
| 召回 | 7 题全部命中 | 每题都召回到分数过线、能用的可比先例 |
| 引用忠实度 | 页码 100% | 保留先例的每条引用，翻到源文件那一页都一字不差 |
| 初筛准确率 | 63 条主题不符候选全部挡掉 | 主题对不上立即淘汰，判定没有出错 |
| 可用率 | 15 条留存中 12 条经人工复核可用 | 另外 3 条主题相关但口径偏了，已标为误留 |
| 成本 | 7 题合计约 ¥2.08 | 平均每题约 ¥0.3，绝大部分输入命中上下文缓存 |
| 耗时 | 平均约 5 分半一题 | 全程没有爆上下文 |

最值得说的是召回。有两题人工一条先例都没翻到，工具各捞出一条确实能用的；也有一题工具漏掉了库里本来就有的一条可用先例。它能补人工的盲点，也有自己的盲区，两边都照实记下来。所有被淘汰、被丢弃的候选都留在 `_work/ranking_report.jsonl`，可以逐条回看判得对不对。
