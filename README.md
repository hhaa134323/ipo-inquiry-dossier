...
不依赖完整 Ground Truth，用一套可复核的指标在 7 道真实第一轮审核问询上逐题验证（半导体 IPO，覆盖盈利预测与敏感性、收入与客户、毛利率多口径等难点）。用 DeepSeek V4-Flash 跑完，每题单独计费、人民币按官方定价自算。

| 指标 | 结果 | 说明 |
|---|---|---|
| 命中率 | 7/7（100%） | 每题都召回到通过评分阈值的可比先例 |
| 溯源页码忠实度 | 100% | 保留先例的每条引用，翻到源文件页码一字不差可核 |
| 粗筛判定（闸一） | 63/63 正确 | 主题不符（评分维度 A=0）立即淘汰，判定全部正确 |
| 保留精度 | 12/15（80%） | 保留候选里经人工复核可用占八成；3 条高分候选主题相关但口径偏离，被标为误留并如实记录 |
| 成本 | 7 题累计约 ¥2.08 | 平均每题约 ¥0.3，绝大部分输入命中上下文缓存 |
| 耗时 | 平均约 5.5 分钟/题 | 全程无爆上下文 |

**召回增量**：其中两题人工一条先例都没找到，工具各捞出一条确实能用的先例；也有一题工具漏掉了库里确实存在的一条可用先例。召回既有真实增量，也如实暴露盲点——所有被淘汰、被丢弃的候选都留痕在 `_work/ranking_report.jsonl`，可逐条回溯。
